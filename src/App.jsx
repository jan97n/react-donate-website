...
const mainnetProvider = new JsonRpcProvider("https://ropsten.infura.io/v3/8ead0a3799ee4378b36cb87482c87e2e");
...
const localProvider = new JsonRpcProvider("https://ropsten.infura.io/v3/8ead0a3799ee4378b36cb87482c87e2e"); //mainnetProvider//
...
function App(props) {
  ...
  if (roundFinished) {
    ...
    status = (
      ...
      >
        Funding round is <span style={{ color: "#95de64" }}>open</span>, ends in{" "}
        <span style={{ color: "#adc6ff" }}>{humanizeDuration(roundFinishedIn * 1000)}</span>.
      </div>
    );
  } else {
    status = (
    ...
    <div className="App">
      ...
      <BrowserRouter>
        <Menu style={{ textAlign: "center" }} selectedKeys={[route]} mode="horizontal">
          ...
          <Menu.Item key="/code">
            <a target="_blank" href="https://github.com/austintgriffith/scaffold-eth/tree/emoji-support">
              🍴
            </a>
          </Menu.Item>
          <Menu.Item key="/chat">
                    ...
                    <List.Item key={index}>
                      <div>
                        <div style={{ textAlign: "left", padding: 8, fontWeight: "bolder", letterSpacing: "1.5px" }}>
                          ...
                          <a style={{ fontSize: 8 }} href={item.link}>
                            {"🔗"}
                          </a>
                        </div>
                      ...
                      <div style={{ float: "right", opacity: roundFinished ? 0.1 : 1 }}>
                        <Row>
                          ...
                          <Col span={8}>
                            <Button
                              onClick={() => {
                                if (supportAmounts && Number(supportAmounts[index])) {
                                  tx(
                                    writeContracts.MVPCLR.donate(index, {
                                      value: parseEther("" + Number(supportAmounts[index]).toFixed(8)),
                                    }),
                                  );
                                  let current = supportAmounts;
                                  current[index] = "";
